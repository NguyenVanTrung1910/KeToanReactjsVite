import React, { FC, memo, ReactNode, useCallback, useContext, useState, useRef } from 'react';
...
const Item: FC<IItemProps> = ({
	...
	const { tabs, setTabs } = useContext(ThemeContext);
	...
	const removeTab = (tabId: string) => {
		setTabs((prevTabs) => {
			...
			delete updatedTabs[tabId]; // Xóa tab theo ID
	...
	if (isHorizontal) {
		return (
			<li
				...
				style={{ position: "relative", paddingRight: "5px"}} // Đảm bảo <li> là relative
			>
				{WITHOUT_CHILD}
				{isHorizontal && isHovered && (
					<span
						onClick={() => removeTab(id || "no")}
						style={{
							color: "red",
							cursor: "pointer",
							position: "absolute",
							right: "10px", // Căn phải
							top: "50%", // Đưa xuống 50% chiều cao
							transform: "translateY(-50%)", // Kéo lên lại 50% để căn giữa
							zIndex: 5,
						}}
					>
						X
